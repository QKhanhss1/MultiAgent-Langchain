...
@tool
def create_event(access_token: str, summary: str, start_time: str, end_time: str, user_id: int, description: Optional[str] = None, attendees: Optional[List[str]] = None) -> str:
    """
    Tạo một sự kiện mới trong lịch chính.
    'access_token' là Google OAuth access token.
    'summary' là tiêu đề của sự kiện.
    'start_time' và 'end_time' phải có định dạng ISO 8601.
    'user_id' là ID người dùng cho InCard app.
    """
    try:
        service = get_calendar_service_with_token(access_token)
        event_body = {
...
@tool
def update_event(access_token: str, event_id: str, user_id: int, new_summary: Optional[str] = None, new_start_time: Optional[str] = None, new_end_time: Optional[str] = None, new_description: Optional[str] = None,  new_attendees: Optional[List[str]] = None) -> str:
    """
...
@tool
def delete_event(access_token: str, event_id: str, user_id: int) -> str:
    """
